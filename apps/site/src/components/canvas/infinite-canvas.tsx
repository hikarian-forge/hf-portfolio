...
export function InfiniteCanvas() {
	...
	const updateItemPosition = useCanvasStore(
	...
	useEffect(() => {
		...
		const handlePointerMove = (event: PointerEvent) => {
			...
			if (interaction.mode === "move-item") {
				const deltaX =
					(event.clientX - interaction.pointerX) / controllerRef.current.scale;
				const deltaY =
					(event.clientY - interaction.pointerY) / controllerRef.current.scale;
				updateItemPosition(
					interaction.itemId,
	...
	useEffect(() => {
		...
		const handleKeyDown = (event: KeyboardEvent) => {
			...
			if (event.key === "Backspace") {
				const activePanel = openPanels.find(
	...
	const centerX = Math.round(
		controllerRef.current.toVirtualX(viewportSize.width / 2),
	);
	const centerY = Math.round(
		controllerRef.current.toVirtualY(viewportSize.height / 2),
	);
										...
										<div className="space-y-2" key={`group-${groupIndex}`}>
											{group.map(([keys, description]) => (
												<div
													...
													key={keys}
												>
													<span className="rounded-md bg-background px-2 py-1 font-mono text-xs text-foreground">
														{keys}
													</span>
			...
			<div
				...
				onWheel={(event) => {
					...
					if (event.ctrlKey || event.metaKey) {
						...
						controllerRef.current.zoomAtPoint(
