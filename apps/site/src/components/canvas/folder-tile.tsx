...
export function FolderTile({ folder, isSelected, onPointerDown, onClick, onDoubleClick }: FolderTileProps) {
  return (
    <button
      type="button"
      className={cn(
      ...
      style={{ left: folder.x, top: folder.y, width: folder.width, minHeight: folder.height }}
    >
      ...
      <span className="max-w-full truncate text-[13px] font-medium leading-4 text-foreground">{folder.title}</span>
    </button>
  )
}
