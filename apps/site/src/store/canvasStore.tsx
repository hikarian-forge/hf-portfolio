...
export interface CanvasState extends ViewportState {
	...
	openFile: (fileId: string, variant?: TileVariant) => void;
	openFolder: (folderId: string, variant?: TileVariant) => void;
	...
	updatePanelVariant: (panelId: string, variant: TileVariant) => void;
...
const updateFolderChildrenLayouts = (
	items: FolderChild[],
	layouts: ItemLayoutMap,
): FolderChild[] =>
	items.map((item) => {
...
const applyLayoutsToCanvasItems = (
	items: CanvasItem[],
	layouts: ItemLayoutMap,
): CanvasItem[] =>
	items.map((item) => {
...
const findItemLocationInChildren = (
	...
	for (const item of items) {
		...
		if (item.type === "folder" && depth < 5) {
			const nested = findItemLocationInChildren(
				item.contents,
				itemId,
				nextPath,
				item.id,
...
const buildPanelFromLocation = (
	...
	const bounds = base ?? {
		...createPanelBounds(index, variant),
		zIndex: index + 1,
	};
...
const withPanelFocused = (
	panels: OpenPanel[],
	panelId: string,
): OpenPanel[] => {
	const currentMax = panels.reduce(
		...
		buildPanelFromLocation(
			{
				...
				item: {
					...location.item,
					panelVariant: variant ?? location.item.panelVariant,
				},
			},
			null,
...
const initialFolders = applyLayoutsToCanvasItems(
	canvasFolders,
	{},
) as FolderTile[];
	...
	persist(
		(set, get) => ({
			...
			updateItemPosition: (itemId, x, y) =>
				set((state) => {
					...
					return {
						itemLayouts,
						files: updateCanvasItems(state.files, itemId, (item) => ({
							...item,
							x,
							y,
						})) as FileTile[],
						folders: updateCanvasItems(state.folders, itemId, (item) => ({
							...item,
							x,
							y,
						})) as FolderTile[],
			...
			openFile: (fileId, variant) => {
				...
				set((state) => {
					const openPanels = openPanelForLocation(
						location,
						state.openPanels,
						variant,
					);
			...
			closePanel: (panelId) =>
				set((state) => {
					...
					const openPanels = state.openPanels.filter(
			...
			updatePanelPosition: (panelId, x, y) =>
				set((state) => {
					const snapped = getSnappedPanelPosition(
						state.openPanels,
						panelId,
						x,
						y,
			...
			updatePanelVariant: (panelId, variant) =>
				set((state) => {
					const openPanels = state.openPanels.map((panel) =>
						panel.id === panelId ? { ...panel, variant } : panel,
					);
					return {
						openPanels,
						persistedPanels: serializePanels(openPanels),
		...
		}),
		{
			...
			merge: (persistedState, currentState) => {
				...
				const files = applyLayoutsToCanvasItems(
					canvasFiles,
					itemLayouts,
				) as FileTile[];
				const folders = applyLayoutsToCanvasItems(
					canvasFolders,
					itemLayouts,
				) as FolderTile[];
